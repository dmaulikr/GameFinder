...
- (IBAction)handlePathButtonPressed:(id)sender {
    PFGeoPoint *geoPoint = self.placeObject[@"location"];
    ...
    UIAlertAction *ok = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        CLLocationCoordinate2D placeLocation = CLLocationCoordinate2DMake(geoPoint.latitude, geoPoint.longitude);
        MKPlacemark *placemark = [[MKPlacemark alloc] initWithCoordinate:placeLocation addressDictionary:nil];
        MKMapItem *item = [[MKMapItem alloc] initWithPlacemark:placemark];
        item.name = self.placeObject[@"name"];
